...
public class OBDService: ObservableObject, OBDServiceDelegate {
  ...
  /// - Returns: true or false if the adapter connected
  /// - Throws: Errors that might occur during the connection process.
  public func connectToAdapter(timeout: TimeInterval = 7) async throws -> Bool {
    do {
  ...
  public func connectToVehicle(preferedProtocol: PROTOCOL? = nil) async throws -> OBDInfo {
    do {
      ...
      throw OBDServiceError.vehicleConnectionFailed(underlyingError: error)
    }
  }
  ...
  /// - Throws: Errors that might occur during the request process.
  public func requestPIDs(_ commands: [OBDCommand], unit: MeasurementUnit) async throws -> [OBDCommand:
    MeasurementResult]
  {
  ...
  ///  - Returns: The raw response from the vehicle.
  ///  - Throws: Errors that might occur during the request process.
  public func sendRawCommand(_ command: OBDCommand) async throws -> [String] {
    do {
      return try await sendCommand(command.properties.command)
    } catch {
      throw OBDServiceError.commandFailed(command: command.properties.command, error: error)
    }
  }
...
public enum OBDServiceError: Error {
  ...
  case vehicleConnectionFailed(underlyingError: Error)
