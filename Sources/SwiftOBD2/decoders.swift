...
class UAS {
  ...
  func decode(bytes: Data, _ unit_: MeasurementUnit = .metric) -> MeasurementResult {
    ...
    if unit_ == .imperial {
      ...
      print("Hey, we are converting \(scaledValue) \(self.unit)")
    }
...
public enum Decoders: Equatable {
  ...
  func performDecode(data: Data, unit: MeasurementUnit = .metric) -> Result<DecodeResult, DecodeError> {
    ...
    case .throttlePos:
      return throttlePositionDecoder(data)
    case .obdCompliance:
...
func fuelRateDecoder(_ data: Data) -> Result<DecodeResult, DecodeError> {
  ...
  return .success(
    (.measurementResult(MeasurementResult(value: value, unit: UnitFuelEfficiency.litersPer100Kilometers))))
}
...
func voltageDecoder(_ data: Data) -> Result<DecodeResult, DecodeError> {
  ...
  return .success(
    .measurementResult(MeasurementResult(value: voltage, unit: UnitElectricPotentialDifference.volts)))
}
...
// 0 to 100% Throttle Position
func throttlePositionDecoder(_ data: Data) -> Result<DecodeResult, DecodeError> {
  let value = (Int(data.first ?? 0) * 100) / 255
  return .success(.measurementResult(MeasurementResult(value: Double(value), unit: Unit.percent)))
}
...
func airStatusDecoder(_ data: Data) -> Result<DecodeResult, DecodeError> {
  ...
  if numSet == 1 {
    ...
    return .success(
      .measurementResult(MeasurementResult(value: Double(index), unit: UnitElectricCurrent.amperes)))
  }
  return .failure(.invalidData)
}
